...
// Point state at a throwaway dir BEFORE importing (config.ts reads the env at
// import time). Another test file may have imported config first in the same
// run, so derive the ACTUAL file path from config instead of assuming ours won.
process.env.TG_TOPICS_STATE_DIR ||= mkdtempSync(join(tmpdir(), "tgsent-"));
